...
class ArtController < ApplicationController
	...
	def index
		...
		@category = ArtCategory.friendly.find( params[:category] || params[:cat] ) if ( params[:category] || params[:cat] ).present?

		@title = @category.try(:name)
		@title ||= "Pictures"

		@pictures = Art.active.order( publish_at: :desc )
	...
	def show
		...
		@pictures = Art.where( "availability <= :level", level: @user_level )
